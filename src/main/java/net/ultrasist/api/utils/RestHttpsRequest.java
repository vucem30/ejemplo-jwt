...
public class RestHttpsRequest {
    ...
    public String checkCaptcha(String response) {
        return checkCaptcha(GOOGLE_RECAPTCHA, response);
    }
    ...
    public boolean invalidJwt(String jwt) {
        return this.sendGetRequest(ACCESS + "verify-jwt/"+jwt).contains("null@mail.com");
    }

    private String sendGetRequestBase(String url, String jwt) {
        Map<String, String> headers = new HashMap<>();
        headers.put("jwt", jwt);
        return sendGetRequestBase(url, headers);
    }
    ...
    private String sendGetRequestBase(String url, Map<String, String> headers) {
        Builder prev = HttpRequest.newBuilder()
            .uri(URI.create(url))
            .header(CT, "application/json");
            if(headers!=null && headers.size()>0) {
